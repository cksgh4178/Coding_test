...
def construct_heap(jobs, timeline):
    heap = []
    for job in jobs:
        priority = job[-1] if job[-2] <= timeline else 9999
        heapq.heappush(heap, (priority, job[-2], job[-1]))
    return heap
...
def solution(jobs):
    ...
    timeline = 0
    total = 0
    heap = construct_heap(jobs, timeline)
    while len(heap) > 0:
        (_, req, wt) = heap.pop(0)
        if req > timeline:
            total += wt
            timeline += (req-timeline) + wt
        else:
            total += (timeline + wt) - req
            timeline += wt
        heap = construct_heap(heap, timeline)
    answer = int(total/n)
    return answer
```

힙을 사용하여 SJF를 구현하면 되는 문제.

처음에는 문제 예시에 맞도록 완벽한 SJF로 구현하지 않아서 계속 틀리는 부분이 발생하였다.

heapq에서 정렬은 제일 앞에 있는 원소를 기준으로 일어나게 되므로 construct_heap 함수를 이용하여

heap을 새롭게 구성하였다.

한 작업이 끝나게 되면 그 때 남은 작업 중 가장 짧은 작업을 다시 구하기 위해 새롭게 힙을 구성한다.